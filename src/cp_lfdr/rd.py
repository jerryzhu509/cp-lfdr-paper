    ...
    *,
    ...
    for level_id, group in data.groupby(level):
        ...
        comparison_tolerance = (
            10
            * np.finfo(float).eps
            * max(1.0, abs(float(observed_local)))
        )
        if is_exact:
            local_pvalue = np.mean(
                local_distribution
                >= observed_local - comparison_tolerance
            )
        else:
            local_pvalue = (
                np.sum(
...
def construct_year_specific_midpoint_base(
    data,
    *,
    town_column="town",
    year_column="Y",
    ...
    """Build translation-invariant midpoint distances within each cohort.

    Cutoffs are ordered by their recovered positions separately within every
    town-year.  For adjacent cutoffs ``a`` and ``b``, a student's distance to
    their midpoint is exactly ``(d_a + d_b) / 2``.  This expression uses only
    the two observed cutoff distances and is therefore invariant to the
    unidentified location of the admission-score coordinate in each year.

    A placebo construction is identified by its town and the unordered pair
    of adjacent school/track labels.  The same pair is therefore pooled across
    the cohorts in which those labels remain adjacent, even if its ordinal
    position among the town's cutoffs changes.
    """
    required = {
        town_column,
        year_column,
    ...
    frame[distance_column] = (
        frame[distance_column].astype(float).round(2)
    )

    cutoff_table = (
        frame.groupby(
            [town_column, year_column, cutoff_column],
            as_index=False,
        )[distance_column]
        .median()
        .rename(columns={distance_column: "median_distance"})
    )
    cutoff_table["cutoff_position"] = (
        -cutoff_table["median_distance"].astype(float)
    ).round(6)
    cutoff_table = cutoff_table.sort_values(
        [town_column, year_column, "cutoff_position", cutoff_column]
    ).reset_index(drop=True)
    cutoff_table["cutoff_rank"] = (
        cutoff_table.groupby([town_column, year_column]).cumcount()
        + 1
    )

    year_gaps = cutoff_table.rename(
        columns={
            cutoff_column: "z_left",
            "cutoff_position": "left_cutoff_position",
            "cutoff_rank": "gap_rank",
        }
    )[
        [
            town_column,
            ...
            "gap_rank",
            "z_left",
            "left_cutoff_position",
        ]
    ].copy()
    grouped = cutoff_table.groupby([town_column, year_column])
    year_gaps["z_right"] = grouped[cutoff_column].shift(-1)
    year_gaps["right_cutoff_position"] = grouped[
        "cutoff_position"
    ].shift(-1)
    year_gaps = year_gaps.dropna(
        subset=["z_right", "right_cutoff_position"]
    ).copy()
    year_gaps["z_right"] = year_gaps["z_right"].astype(int)
    year_gaps["gap_width"] = (
        year_gaps["right_cutoff_position"]
        - year_gaps["left_cutoff_position"]
    ).round(6)
    year_gaps["midpoint_position"] = (
        (
            year_gaps["left_cutoff_position"]
            + year_gaps["right_cutoff_position"]
        )
    ...
    year_gaps["z_low"] = year_gaps[["z_left", "z_right"]].min(
        axis=1
    ).astype(int)
    year_gaps["z_high"] = year_gaps[["z_left", "z_right"]].max(
        axis=1
    ).astype(int)
    year_gaps["placebo_id"] = (
        year_gaps[town_column].astype(str)
        + "_placebo"
        + year_gaps["z_low"].astype(str)
        + "_"
        + year_gaps["z_high"].astype(str)
    )

    left_rows = frame.merge(
        year_gaps[
            [
                town_column,
                year_column,
        ...
        left_on=[town_column, year_column, cutoff_column],
        right_on=[town_column, year_column, "z_left"],
        how="inner",
        validate="many_to_one",
    ).rename(columns={distance_column: "left_distance"})
    right_rows = frame[
        [
            town_column,
            year_column,
        ...
        columns={
            cutoff_column: "z_right",
            distance_column: "right_distance",
        }
    )
    paired = left_rows.merge(
        right_rows,
        on=[town_column, year_column, student_column, "z_right"],
        how="inner",
        validate="one_to_one",
    )
    paired["observed_gap_width"] = (
        paired["left_distance"] - paired["right_distance"]
    ).round(6)
    maximum_gap_error = (
        paired["observed_gap_width"] - paired["gap_width"]
    ).abs().max()
    if maximum_gap_error > numerical_tolerance:
        raise ValueError(
            "Student-level cutoff spacings disagree with the recovered "
            f"cutoff table (maximum error {maximum_gap_error})."
        )
    paired["placebo_distance"] = (
        (paired["left_distance"] + paired["right_distance"])
        / 2
    ).round(6)
    paired = paired.rename(columns={outcome_column: "outcome"})
    paired = paired[
        [
            town_column,
            year_column,
    ...
    return cutoff_table, year_gaps, paired
